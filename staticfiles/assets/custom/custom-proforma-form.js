...
$(document).ready(function () {
	...
	for (var i = 0; i < currentFormCount; i++) {
		$(`#id_proforma_goods-${i}-unit`).select2({
			minimumResultsForSearch: Infinity,
		});
	}
});
...
const totalNewForms = document.getElementById("id_proforma_goods-TOTAL_FORMS");
...
addMoreBtn.addEventListener("click", (event) => {
	...
	copyEmptyFormEl.setAttribute(
		"class",
		"row goods-list dynamic-proforma_goods"
	);
	...
	const productElement = document.getElementById(
		`div_id_proforma_goods-${updatedFormCount}-product`
	);
	const qtyElement = document.getElementById(
		`div_id_proforma_goods-${updatedFormCount}-quantity`
	);
	const unitElement = document.getElementById(
		`div_id_proforma_goods-${updatedFormCount}-unit`
	);
	const unitPriceElement = document.getElementById(
		`div_id_proforma_goods-${updatedFormCount}-unit_price`
	);
	const deleteElement = document.getElementById(
		`div_id_proforma_goods-${updatedFormCount}-DELETE`
	);

	productElement.setAttribute("class", "col-sm-12 col-md-6 col-lg-3 mb-3");
	...
	var forAttributeValue = `id_proforma_goods-${updatedFormCount}-DELETE`;
	var labelElement = document.querySelector(
		'label[for="' + forAttributeValue + '"]'
	);
	labelElement.style.display = "none";

	$(`#id_proforma_goods-${updatedFormCount}-unit`).select2({
		minimumResultsForSearch: Infinity,
	});
