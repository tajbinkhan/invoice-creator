...
def list_bill_invoice_api(request):
	invoice_list = BillInvoice.objects.all().prefetch_related(Prefetch('bill_goods', queryset=BillInvoiceGoods.objects.all().order_by('id')))
...
def detail_invoice(request, invoice_id):
	...
	bill_invoice = BillInvoice.objects.prefetch_related('bill_goods').get(invoice_id=invoice_id)
...
def update_invoice(request, invoice_id):
	...
	if request.method == 'POST':
		...
		if form.is_valid() and formset.is_valid():
			...
			formset.instance = invoice
			formset.save()
			messages.success(request, f'{bill_invoice_id.invoice_number} updated successfully!')
			return redirect('bill_invoice')
	else:
...
def delete_invoice(request, invoice_id):
	...
	if request.method == "POST":
		...
		messages.success(request, f'{bill_invoice_id.invoice_number} deleted successfully!')
		return redirect('bill_invoice')
