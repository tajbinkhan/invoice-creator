...
class ProformaInvoiceForm(forms.ModelForm):
	class Meta:
		...
		widgets = {
			...
			"client": forms.Select(
				attrs={"class": "clientpicker", "data-placeholder": "Select a client"}
			),
			"custom_total_amount": forms.TextInput(
				attrs={
...
class ProformaInvoiceGoodsForm(forms.ModelForm):
	class Meta:
		...
		exclude = ("id",)
		widgets = {
			"product": forms.TextInput(attrs={"placeholder": "Enter product name"}),
			"quantity": forms.TextInput(
				attrs={"placeholder": "Enter product quantity"}
			),
			"unit_price": forms.TextInput(
				attrs={"placeholder": "Enter product unit price"}
			),
			"unit": forms.Select(
				attrs={"class": "unitpicker", "data-placeholder": "Select a unit"}
			),
		}
